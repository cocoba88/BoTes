...
var handler = async (m, { text, usedPrefix, command }) => {
    ...
    try {
        ...
        // Making requests to all three APIs
        const apiEndpoints = [
            `https://aemt.me/gpt4?text=${encodeURIComponent(text)}`,
            `https://aemt.me/gemini?text=${encodeURIComponent(text)}`,
            `https://aemt.me/bard?text=${encodeURIComponent(text)}`,
            `https://aemt.me/openai?text=${encodeURIComponent(text)}`,
            `https://aemt.me/v2/turbo?text=${encodeURIComponent(text)}`
        ];

        const responses = await Promise.all(apiEndpoints.map(url => axios.get(url)));

        // Extracting response data from each API
        ...
        // Sending the responses along with the randomly selected video
        for (const result of results) {
            if (result) {
                ...
                m.reply("Error getting response from one of the APIs.");
            }
        }
    } catch (error) {
        console.error(error);
...
handler.command = handler.help = ['chtx', 'gemini', 'turbo'];
handler.tags = ['tools'];
